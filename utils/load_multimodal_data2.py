...
def load_multimodal_data(gt_path, *src_path, patch_size=(7, 7), is_labeled=True, nb_comps, device):
    p = Processor()
    ...
    # x = torch.concat(x, dim=1).to(device)
    
    # _, n = x.shape
    pca = PCA(nb_comps) #pca
    
    modality_list[0] = pca.fit_transform(modality_list[0])
    print('pca shape: %s, percentage: %s' % (modality_list[0].shape, np.sum(pca.explained_variance_ratio_)))
    x = []
    for x_ in modality_list:
        x_ = torch.tensor(x_).t().to(device)
        x.append(x_)
    # x = x.cpu().numpy()
