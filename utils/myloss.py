...
class net(nn.Module):
    ...
    def __init__(self, spatial_size, num_anchor, A1, A2, kernel='rbf', smooth_coef=1.2, lambda_tv=1., init_z=None):
        """
    ...
    def forward(self, x_list, anchors, modality_weights, update):
        
        self.A1.requires_grad=update
        self.A2.requires_grad=update
        self.Z.requires_grad = not self.A1.requires_grad
        
        sum_ = 0.
        # for a_, x_, mu_ in zip(anchors, x_list, modality_weights):
        sum_ += 0.5 * torch.norm(x_list[1] - torch.matmul(self.A2, self.Z), p='fro').pow(2) * modality_weights[1].pow(self.smooth_coef)
        sum_ += 0.5 * torch.norm(x_list[0] - torch.matmul(self.A1, self.Z), p='fro').pow(2) * modality_weights[0].pow(self.smooth_coef)
